...
def claim_r4():
    ...
    for sh in ("w132", "w264", "w1584"):
        ...
        # ⛔ Bai in ti le nay dang PHAN TRAM ("27.2% cua khe 60s") con CSV ghi dang phan so.
        # Truoc do bai go tay ca hai, va chung lech nhau: 27.2% so voi 0.273 = 27.3%.
        # Sinh rieng mot claim phan tram de van bai khong phai tu nhan doi mot con so.
        claim_raw(f"time_pct_{sh}", "r4_4_stage_timing.csv", "frac_of_slot",
                  {"shell": sh}, "median", 1, f"phan tram khe 60s tren {sh}",
                  scale="x*100")
...
def claim_inherited():
    ...
    # ⛔ Baseline the-nang hoc duoc (p10) TRUOC DAY khong co claim nao, nen bai go tay
    # "0.74" va "0.92" cho no. Hau qua lo ra khi noi so vao macro: gia tri 0.92 trung voi
    ...
    for split, tag in (("in-dist", "indist"), ("ood", "ood")):
        for cid, col, pl in ((f"p10_rel_learned_{tag}", "r_learned", 2),
            ...
            claim_raw(cid, "p10_learned.csv", col, {"split": split}, "mean", pl,
                      f"baseline the-nang hoc duoc: {col}, {split}")
