...
\item Enforcing capacity feasibility reduces the headline gap from $80.5\%$ measured as delay to
$29.8\%$ measured as delivered rate, stable across three loss models (new Section VI-G).
...
\item Before submitting we had the manuscript read by an independent reviewer outside the
project, and acted on that reading as well. Four further controls were run and three further
claims moved: the speedup is restated against a warm-started solver, which needs $2$ to $6$
iterations rather than $20$ and cuts the factor from $21$--$29\times$ to roughly $2$--$9\times$;
the proactive variant is shown to lose to the blind multipath baseline at every drift level on an
unseen shell; the $1584$-satellite shell finally receives the temperature sweep and the blind
comparator that the rest of the paper demands, and the blind baseline comes out ahead there. We
also added a fourth evaluation pitfall (Section V-A of the manuscript): the
recovered-fraction metric saturates when its blind reference is pathological, so four points of
that fraction can separate a policy two percent from equilibrium from one more than a hundred
times away. Reviewer 1's observation about numbers disagreeing across tables turned out to have a
second, larger instance than the one spotted, and the manuscript now pools the run sets and
reports the range.
...
narrowed to the four specific pitfalls we measured. No section was deleted outright; the material
...
costs a median of $0.077$, $0.066$ and $0.053$ per instance on three unseen shells. Because that
...
filter and aggregation that produce it, and a script recomputes all $97$ of them. Your comment also
...
$0.668$, paired difference $0.310$) and level outside it ($0.984$/$0.976$, $0.974$/$0.973$,
...
regime. Recomputing the same configurations under a hard capacity, the advantage reads $80.5\%$ as
a reduction in travel time and $29.8\%$ as an increase in delivered rate. Because a loss model is a
choice, we ran three that disagree where it matters: compounding survival along the path gives
$29.8\%$, charging only the bottleneck gives $37.0\%$, and max-min fair sharing gives $37.3\%$. The
